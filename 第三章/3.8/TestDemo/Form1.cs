...
namespace TestDemo
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (pictureBox1.Image != null)
            {
                float delta = (float)Convert.ToInt32(textBox5.Text.ToString());
                float delta_s = (float)Convert.ToInt32(textBox4.Text.ToString());
                DateTime start = DateTime.Now;
                curBitmap = ImageProcessDll.BeepsFilter(srcBitmap, delta, delta_s);
