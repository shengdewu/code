...
namespace TestDemo
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (pictureBox1.Image != null)
            {
                int iter = Convert.ToInt32(textBox5.Text.ToString());
                float K = (float)Convert.ToDouble(textBox4.Text.ToString());
                float lambda = (float)Convert.ToDouble(textBox3.Text.ToString());
                DateTime start = DateTime.Now;
                curBitmap = ImageProcessDll.AnisotropicFilter(srcBitmap, iter, K,lambda);
