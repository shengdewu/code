...
{
	return f_Gender_MNN_Initialize(genderModelPath, stdMode, numThread);
};
void SF_Gender_MNN_Unitialize()
{
	f_Gender_MNN_Unitialize();
};
int SF_FaceGender(unsigned char* srcData, int width, int height, int stride, float outGender[2])
{
	return f_Gender_MNN_Process(srcData, width, height, stride, outGender);
};
