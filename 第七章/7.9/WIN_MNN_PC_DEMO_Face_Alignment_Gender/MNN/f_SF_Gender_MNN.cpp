...
static int TENSOR_SIZEW = 96;
static int TENSOR_SIZEH = 116;
static int STD_MODE = 0;
int f_Gender_MNN_Initialize(char* fdmodelPath, int stdMode, int numThread)
{
	int ret = 0;
	TENSOR_SIZEW = 96;
	TENSOR_SIZEH = 116;
...
inline int MNN_Process(unsigned char* srcData, int width, int height, int stride, float outGender[2])
{
	int ret = 0;
	...
	if (STD_MODE == 0)
	{
		...
		const float norms[3] = { 1.0f,1.0f,1.0f };
	...
	CV::Matrix trans;
	trans.postScale((float)TENSOR_SIZEW / (width - 1), (float)TENSOR_SIZEH / (height - 1));
	...
	inputdim.push_back(TENSOR_SIZEH);
	inputdim.push_back(TENSOR_SIZEW);
	...
	outGender[0] = data[0];// > 0.5 ? 1 : 0;
	//outGender[1] = data[1] > 0.5 ? 1 : 0;
	return ret;
};

inline void f_GenderImagePadding(unsigned char* srcData, int width, int height, int stride, unsigned char* dstData, int dSize)
{
...
inline void f_GenderImageRePadding(unsigned char* srcData, int width, int height, int stride, unsigned char* dstData, int dSize)
{
	int x0, y0, index_x0y0, index_x0y1, index_x1y0, index_x1y1;
	float disX, disY, tmp1, tmp2;
	unsigned char* pSrc = srcData;
...
int f_Gender_MNN_Process(unsigned char* srcData, int width, int height, int stride, float* outGender)
{
	...
	//f_GenderImagePadding(srcData, width, height, stride, dstData, TENSOR_SIZE);
	int ret = MNN_Process(srcData, TENSOR_SIZEW, TENSOR_SIZEH, TENSOR_SIZEW * 4, outGender);
	return ret;
}
